        ...
        <div className="overflow-y-auto max-h-[600px] space-y-3 pr-1 scrollbar-thin scrollbar-thumb-yellow-400/60 scrollbar-track-transparent">
          {casesinQueue
            ...
            .map((case_) => {
              ...
              // Diferencia de días (redondeada hacia arriba)
              ...
              // --- NUEVAS VARIABLES ---
              const isTomorrow = diffDays === 1;         // Caso para mañana
              let isTwoDaysAhead = diffDays === 2;       // Caso dos días adelante

              // Ajuste de fin de semana (sábado -> martes)
              if (currentDay === 6 && installDay === 2) {
                isTwoDaysAhead = true;
              }

              // Color según el estado temporal
              ...
              return (
                ...
                >
                  {/* Tipo de caso */}
                  <span
                    className={`absolute top-6 right-3 px-2 py-0.5 rounded-md text-[10px] font-bold ${
                      case_.case_type === 'OEM'
